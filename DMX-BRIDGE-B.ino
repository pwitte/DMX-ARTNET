/*
  Bridge_B_ArtNet_Fixed.ino
  Bridge B: DMX <-> Art-Net Bridge (ESP32-WROOM)

  - ESP32 core: 2.0.17
  - esp_dmx:    4.1.0
  - Safe EN pins (no boot loops)
  - DMX → ArtNet by default
  - Universes: DMX1 = 2, DMX2 = 3
  - Web UI for config
...
// ---------- Wi-Fi AP defaults ----------
const char* AP_NAME = "DMX-Bridge-B";
...
// Port 1 default config: Universe 2
uint16_t cfg_uni1 = 2;
...
// Port 2 default config: Universe 3
uint16_t cfg_uni2 = 3;
...
<title>DMX Bridge B</title>
</head><body>
<h2>DMX Bridge B Configuration</h2>
...
// ---------- Correct ArtNet Sender ----------
void sendArt(uint16_t uni, uint8_t* data, uint16_t len){
  // IMPORTANT FIX:
  // ArtNet DMX length must be 2–512 AND EVEN.
  if(len < 2) return;
  if(len > 512) len = 512;
  if(len & 1) len--;      // enforce even
  if(len < 2) return;
  ...
  IPAddress bc;
  if(bc.fromString(cfg_broadcast)) artnetUdp.beginPacket(bc, 6454);
  else                             artnetUdp.beginPacket(IPAddress(255,255,255,255), 6454);
